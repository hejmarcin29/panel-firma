...
export function buildClientBasedKey(opts: {
  ...
  const v = typeof opts.version === "number" ? String(opts.version).padStart(2, "0") : "01"
  const ext = safeExtFromFilename(opts.filename)
  const base = title ? `${datePart}__${title}__v${v}` : `${datePart}__v${v}`
  const nameCore = opts.includeClientInFilename ? `${base}__${slugify(opts.clientName)}` : base
