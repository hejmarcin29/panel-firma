...
export async function getInstallationDeliveriesSnapshot(limit = 5): Promise<InstallationDeliveriesSnapshot> {
...
export async function getInstallationDeliveriesList(limit = 30): Promise<InstallationDeliveryListItem[]> {
  const rows = await db
    .select({
      id: deliveries.id,
      deliveryNumber: deliveries.deliveryNumber,
...
export async function getInstallationDeliveriesDashboardData(
  ...
  const [snapshot, list] = await Promise.all([
    getInstallationDeliveriesSnapshot(limit),
    getInstallationDeliveriesList(limit),
  ]);
