...
import { MontageChecklistTemplate, DEFAULT_MONTAGE_CHECKLIST } from './checklist-shared';

export type { MontageChecklistTemplate };
export { DEFAULT_MONTAGE_CHECKLIST };
...
export async function setMontageChecklistTemplates({ templates, userId }: PersistChecklistParams): Promise<void> {
	...
	const payload = deduped
		...
		.map(({ id, label, allowAttachment, associatedStage }) => ({ id, label, allowAttachment, associatedStage }));
