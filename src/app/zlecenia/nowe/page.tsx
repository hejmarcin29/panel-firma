...
export default function NoweZlecenieSelectPage() {
  ...
  async function handleCreateDelivery() {
    ...
    if (!plannedAt) {
      toast({
        title: "Błąd",
        description: "Podaj datę dostawy",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const placedTs = orderPlacedAt
        ? new Date(orderPlacedAt + "T00:00:00").getTime()
        : undefined;
      ...
      if (!createOrderRes.ok)
        throw new Error(created?.error || "Nie udało się utworzyć zlecenia");
      ...
      const lines = items
        ...
        .map(
          (it) =>
            `• ${it.name.trim()}${it.sqm ? ` – ${it.sqm} m²` : ""}${it.packs ? ` (${it.packs} op.)` : ""}`,
        );
      ...
      if (!slotRes.ok)
        throw new Error(slotJson?.error || "Nie udało się dodać dostawy");
    ...
    <div className="mx-auto max-w-4xl p-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-semibold">
          {pl.orders?.new || "Nowe zlecenie"}
        </h1>
      </div>
      <p className="text-sm opacity-70">
        {fromClientId
          ? "Wybierz rodzaj zlecenia dla tego klienta"
          : "Wybierz klienta, a następnie rodzaj zlecenia"}
        .
      </p>
        ...
        >
          <div className="space-y-2">
            <h2 className="text-sm font-medium uppercase tracking-wide opacity-70">
              {fromClientId ? "Rodzaj zlecenia" : "Klienci (ostatni)"}
            </h2>
            {fromClientId ? (
              <div className="rounded border border-black/10 dark:border-white/10 p-3 text-sm">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <Link
                    href={`/zlecenia/montaz/nowy?clientId=${fromClientId}`}
            ...
            <div className="space-y-4">
              ...
              {selectedClient ? (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <Link
                      href={`/zlecenia/montaz/nowy?clientId=${selectedClient.id}`}
                      className="block w-full text-center inline-flex h-10 items-center justify-center rounded-md border border-black/15 hover:bg-black/5 text-sm dark:border-white/15 dark:hover:bg-white/10"
                    >
                      Dodaj montaż
                    </Link>
                    <button
                  ...
                  <p className="text-[11px] opacity-60">
                    Montaż: start od statusu „oczekuje na pomiar”.
                  </p>
                </div>
              ) : (
                <div className="text-xs opacity-60">
                  Najpierw wybierz klienta z listy obok.
                </div>
              )}
            </div>
          )}
      ...
      <AlertDialog
          ...
          <div className="space-y-3 pt-2">
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] opacity-60">
                MVP: na razie tylko Ty jeździsz, więc lista zawiera jedną opcję.
              </p>
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="sameAsInvoice" className="text-xs opacity-70">
        ...
        onConfirm={async () => {
          if (!submitting && canSubmitDelivery) await handleCreateDelivery();
        }}
        showCancelButton
        showConfirmButton
      />
