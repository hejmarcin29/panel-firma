...
const tooltipStore = new WeakMap<
  HTMLElement,
  {
    tooltip: HTMLElement;
    ...
    <div className="p-3">
      <FullCalendar
        ...
        eventDidMount={(arg) => {
          ...
          const kind = (arg.event.extendedProps as { kind?: CalEvent["kind"] })
            .kind;
          ...
          if (kind === "installation") {
            el.style.backgroundColor =
              "color-mix(in oklab, var(--pp-primary) 18%, transparent)";
            el.style.borderColor = "var(--pp-primary)";
          } else if (kind === "delivery") {
            el.style.backgroundColor =
              "color-mix(in oklab, var(--pp-text) 12%, transparent)";
          }
          ...
          tooltip.className =
            "pointer-events-none fixed z-[100] hidden rounded-md border bg-[var(--pp-panel)] p-2 text-sm shadow-lg";
          tooltip.style.borderColor = "var(--pp-border)";
          const parts: string[] = [];
          if (data.clientName || data.orderNo)
            parts.push(
              `<div class="font-medium">${data.clientName ?? ""}${data.orderNo ? ` • ${data.orderNo}` : ""}</div>`,
            );
          if (data.address)
            parts.push(`<div class="opacity-80">${data.address}</div>`);
          if (data.phone)
            parts.push(`<div class="opacity-80">📞 ${data.phone}</div>`);
          if (typeof data.checklistDoneCount === "number")
            parts.push(
              `<div class="mt-1 text-xs">Checklist: ${data.checklistDoneCount} ✓</div>`,
            );
          // Google sync icon
          if (data.googleSync) {
            const icon =
              data.googleSync === "synced"
                ? "✅"
                : data.googleSync === "pending"
                  ? "🕒"
                  : "⚠️";
            parts.push(
              `<div class="mt-1 text-xs">Google: ${icon} ${data.googleSync}</div>`,
            );
          }
          // Conflicts
          if (data.collision && data.colliding && data.colliding.length) {
            const lis = data.colliding
              .map(
                (c) =>
                  `<li><a href="${c.href}" class="underline">${c.orderNo ?? ""}</a> ${c.clientName ?? ""}</li>`,
              )
              .join("");
            parts.push(
              `<div class="mt-1 text-xs text-[--pp-warn]">⚠ kolizja</div><ul class="ml-4 list-disc">${lis}</ul>`,
            );
          }
