...
export function CalendarFilters({
  ...
  const applyKinds = (nextKinds: ("installation" | "delivery")[]) => {
    const v =
      nextKinds.length === 0 || nextKinds.length === 2
        ? undefined
        : nextKinds.join(",");
    apply({ kinds: v });
  };
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <DropdownMenu trigger={<span>Filtr: {currentLabel}</span>}>
        ...
        <DropdownItem onSelect={() => apply({ installer: undefined })}>
          Wszyscy montażyści
        </DropdownItem>
        ...
        {installers.map((i) => (
          <DropdownItem
            key={i.id ?? "null"}
            onSelect={() => apply({ installer: i.id ?? undefined })}
          >
            {i.name || "(bez nazwy)"}
          </DropdownItem>
        ))}
      ...
      >
        ...
        <DropdownItem onSelect={() => applyKinds(["installation", "delivery"])}>
          Wszystko
        </DropdownItem>
        <div className="my-1 h-px bg-black/10 dark:bg-white/10" />
        <DropdownItem onSelect={() => applyKinds(["installation"])}>
          Tylko montaże
        </DropdownItem>
        <DropdownItem onSelect={() => applyKinds(["delivery"])}>
          Tylko dostawy
        </DropdownItem>
      </DropdownMenu>
      ...
      >
        ...
        <DropdownItem onSelect={() => applyStatuses(["planned", "confirmed"])}>
          Planowane + Potwierdzone
        </DropdownItem>
        <div className="my-1 h-px bg-black/10 dark:bg-white/10" />
        <DropdownItem onSelect={() => applyStatuses(["planned"])}>
          Tylko planowane
        </DropdownItem>
        <DropdownItem onSelect={() => applyStatuses(["confirmed"])}>
          Tylko potwierdzone
        </DropdownItem>
      </DropdownMenu>
      ...
      >
        <button
          aria-label="Widok miesiąc"
          className={`px-2 py-1 rounded ${view === "dayGridMonth" ? "bg-[var(--pp-primary)]/15" : ""}`}
        ...
        <button
          ...
          className={`px-2 py-1 rounded ${view === "dayGridWeek" ? "bg-[var(--pp-primary)]/15" : ""}`}
