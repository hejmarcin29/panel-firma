...
import { db } from '@/db';
import { clients, deliverySlots, installationSlots, orders, users, orderChecklistItems, orderGoogleEvents } from '@/db/schema';
...
type CalendarEvent = {
  ...
  // collisions
  collision?: boolean;
  colliding?: { orderNo?: string | null; clientName?: string | null; href: string }[];
  // google sync
  googleSync?: 'synced'|'pending'|'error';
};
type SearchParams = Promise<{ installer?: string; date?: string; view?: string; range?: string; kinds?: string; statuses?: string; q?: string }>

export default async function CalendarPage({ searchParams }: { searchParams: SearchParams }) {
  const session = await getSession();
  const role = session?.user?.role && isUserRole(session.user.role) ? session.user.role : undefined;
  const userId = session?.user?.id || undefined;
  const { installer = '', date = '', view = '', range = '', kinds: kindsParam = '', statuses: statusesParam = '', q = '' } = await searchParams;
  // If installer role, force own installerId
  const effectiveInstaller = role === 'installer' && userId ? userId : installer;
  // Parse filters
  const statusFilter = (statusesParam?.split(',').filter(Boolean) as ('planned'|'confirmed')[]) || [];
  const kindsFilter = (kindsParam?.split(',').filter(Boolean) as ('installation'|'delivery')[]) || [];
  const statusesArr = (statusFilter.length ? statusFilter : (['planned','confirmed'] as const)) as readonly ('planned'|'confirmed')[];
  const showInstallations = kindsFilter.length === 0 || kindsFilter.includes('installation');
  const showDeliveries = kindsFilter.length === 0 || kindsFilter.includes('delivery');
  ...
  const fmtHref = (orderId: string, orderNo: string | null, type: 'delivery'|'installation') =>
    orderNo ? `/zlecenia/nr/${orderNo}_${type === 'installation' ? 'm' : 'd'}` : `/zlecenia/${orderId}`;
  ...
  const dRowsFiltered = (!qNorm || qNorm.length < 2)
    ? dRows
    : dRows.filter(r => (r.clientName ?? '').toLowerCase().includes(qNorm) || (r.orderNo ?? '').toLowerCase().includes(qNorm));
  const iRowsFiltered = (!qNorm || qNorm.length < 2)
    ? iRows
    : iRows.filter(r => (r.clientName ?? '').toLowerCase().includes(qNorm) || (r.orderNo ?? '').toLowerCase().includes(qNorm));

  // Checklist done count per order
  const orderIds = Array.from(new Set([...dRowsFiltered, ...iRowsFiltered].map(r => r.orderId)));
  const checklistCounts = orderIds.length
    ? await db
        .select({ orderId: orderChecklistItems.orderId, done: orderChecklistItems.done })
  ...
  for (const row of checklistCounts) {
    if (row.done) doneByOrder.set(row.orderId, (doneByOrder.get(row.orderId) ?? 0) + 1);
  }

  // Google sync mapping by orderId+installerId
  const googleRows = orderIds.length
    ? await db
        .select({ orderId: orderGoogleEvents.orderId, installerId: orderGoogleEvents.installerId, googleEventId: orderGoogleEvents.googleEventId })
    ...
    ...(role === 'installer' || !showDeliveries
      ...
      : dRowsFiltered
          ...
          .map((r) => ({
            ...
            orderNo: r.orderNo,
            phone: r.phone,
            address: (r.orderLocationAddress || r.clientDeliveryAddress)
  ...
  googleSync: (googleByKey.has(`${r.orderId}::${r.installerId}`) ? 'synced' : 'pending') as 'synced'|'pending',
        // collisions
        ...( (() => {
          const day = toIsoDate(r.plannedAt!)!;
          const key = `${r.installerId ?? 'none'}::${day}`;
          ...
          return others.length > 0 ? {
            ...
            colliding: others.map(o => ({ orderNo: o.orderNo, clientName: o.clientName, href: fmtHref(o.orderId, o.orderNo ?? null, 'installation') }))
          } : {};
        })() ),
      })),
  ];
  ...
  const initialView: 'dayGridMonth' | 'dayGridWeek' = (view === 'dayGridWeek' || range === 'week') ? 'dayGridWeek' : 'dayGridMonth';
    ...
    <div className="mx-auto max-w-none p-4 md:p-6">
      <section className="relative overflow-hidden rounded-2xl border bg-[var(--pp-panel)] mb-4" style={{ borderColor: 'var(--pp-border)' }}>
        ...
        <div className="relative z-10 p-4 md:p-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <h1 className="text-2xl md:text-3xl font-semibold">Kalendarz zleceń</h1>
          <div className="flex flex-wrap items-center gap-2">
            {role === 'installer' ? null : (
              <CalendarFilters
                installers={installers}
                selectedInstaller={effectiveInstaller}
                kinds={kindsFilter.length ? kindsFilter : ['installation','delivery']}
                statuses={statusFilter.length ? statusFilter : ['planned','confirmed']}
                q={q}
                view={initialView}
              />
            )}
      ...
      <div className="rounded-md border" style={{ borderColor: 'var(--pp-border)' }}>
        <CalendarRoot events={events} initialView={initialView} initialDate={initialDate} />
      </div>
    </div>
  );
}
