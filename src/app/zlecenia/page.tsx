...
export default async function OrdersPage({
  ...
  if (installer)
    clauses.push(eq(orders.installerId, installer) as unknown as SQL);
  // outcome filter: active (no outcome), won, lost, all
  let outcomeExpr: SQL | null = null;
  if (outcome === "active")
    outcomeExpr = isNull(orders.outcome) as unknown as SQL;
  else if (outcome === "won")
    outcomeExpr = eq(orders.outcome, "won") as unknown as SQL;
  else if (outcome === "lost")
    outcomeExpr = eq(orders.outcome, "lost") as unknown as SQL;
  ...
  const rows: Row[] = await db
    ...
    .where(
      and(
        ...
        ...(scope === "archived"
          ? [isNotNull(orders.archivedAt) as unknown as SQL]
          : scope === "all"
            ? []
            : [isNull(orders.archivedAt) as unknown as SQL]),
        ...(clauses.length
    ...
    .orderBy(
      // Sorting rules
      ...(sort === "client"
        ? [dir === "asc" ? asc(clients.name) : desc(clients.name)]
        : []),
      ...(sort === "installer"
        ? [dir === "asc" ? asc(users.name) : desc(users.name)]
        : []),
      ...(sort === "delivery"
        ? [
            asc(sql`CASE WHEN nextDeliveryAt IS NULL THEN 1 ELSE 0 END`),
            dir === "asc"
              ? asc(sql`nextDeliveryAt`)
              : desc(sql`nextDeliveryAt`),
          ]
        : []),
      ...(sort === "installation"
        ? [
            ...
            dir === "asc"
              ? asc(sql`nextInstallationAt`)
              : desc(sql`nextInstallationAt`),
          ]
        : []),
      ...(sort === "created" || !sort
        ? [dir === "asc" ? asc(orders.createdAt) : desc(orders.createdAt)]
        : []),
    )
    .limit(limit)
    .offset(offset);
...
function Filters({
  ...
  dir: string;
  scope: string;
  installers: { id: string; name: string | null }[];
}) {
  return (
    <form className="flex flex-wrap gap-2 items-end">
      ...
      <div className="flex flex-col">
        ...
        <input
          name="q"
          defaultValue={q}
          placeholder={pl.orders.searchPlaceholder}
          className="h-9 rounded-md border border-black/15 bg-transparent px-3 text-sm outline-none dark:border-white/15"
        />
      </div>
      <div className="flex flex-col">
      ...
      <button
        type="submit"
        className="h-9 px-3 rounded-md border border-black/15 text-sm hover:bg-black/5 dark:border-white/15 dark:hover:bg-white/10"
      >
        Filtruj
      </button>
    </form>
