...
export default async function CooperationRulesAdminPage() {
  ...
  const rules = await db
    .select({
      id: cooperationRules.id,
      title: cooperationRules.title,
      version: cooperationRules.version,
      isActive: cooperationRules.isActive,
      requiresAck: cooperationRules.requiresAck,
      effectiveFrom: cooperationRules.effectiveFrom,
      createdAt: cooperationRules.createdAt,
    })
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 space-y-6">
      <div className="text-sm opacity-70">
        ...
        &rsaquo; <span>Zasady współpracy</span>
      </div>
      <h1 className="text-2xl font-semibold">Zasady współpracy</h1>

      <Card>
        <CardHeader className="pb-2">
          <CardTitle>Opublikuj nową wersję</CardTitle>
        </CardHeader>
        <CardContent>
          <NewRuleForm />
        </CardContent>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle>Ostatnie wersje</CardTitle>
        </CardHeader>
        <CardContent>
          {rules.length === 0 ? (
              ...
              {rules.map((r) => (
                ...
                >
                  ...
                  <div className="text-xs opacity-70">
                    Utworzono:{" "}
                    {formatDate(r.createdAt as unknown as number, "—")}
                  </div>
                </div>
              ))}
            </div>
