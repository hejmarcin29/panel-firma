      ...
      const body = {
        ...
        effectiveFrom:
          values.effectiveFrom && values.effectiveFrom.trim()
            ? new Date(values.effectiveFrom + "T00:00:00").getTime()
            : undefined,
      };
      const resp = await fetch("/api/zasady-wspolpracy", {
        method: "POST",
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
      <div className="space-y-2">
        ...
        />
        {errors.title && (
          <p id="title-err" className="text-sm text-red-600">
      ...
      <div className="space-y-2">
        ...
        <textarea
          className="min-h-40 w-full rounded-md border border-black/15 bg-transparent px-3 py-2 text-sm outline-none dark:border-white/15"
          {...register("contentMd")}
          aria-invalid={!!errors.contentMd}
          aria-describedby={errors.contentMd ? "content-err" : undefined}
        />
        {errors.contentMd && (
          <p id="content-err" className="text-sm text-red-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            aria-describedby={errors.version ? "version-err" : undefined}
          />
          {errors.version && (
            <p id="version-err" className="text-sm text-red-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            {(["installer", "admin", "architect", "manager"] as const).map(
              (role) => (
        ...
        <div className="space-y-2">
          ...
          <DatePicker
            value={watch("effectiveFrom") || ""}
            onChange={(next) =>
              setValue("effectiveFrom", next, {
                shouldDirty: true,
                shouldValidate: true,
              })
            }
          ...
          <p className="text-xs opacity-70">
            Pozostaw puste, aby obowiązywało od razu.
          </p>
        </div>
      </div>
