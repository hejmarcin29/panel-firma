...
const schema = z.object({
  ...
  invoicePostalCode: z
    ...
    .optional()
    .or(z.literal(''))
    .refine((v) => (typeof v !== 'string') || v === '' || /^[0-9]{2}-[0-9]{3}$/.test(v), {
  ...
  invoiceEmail: z.string().email('Podaj poprawny email').optional().or(z.literal('')),
  ...
  if (data.buyerType === 'company') {
    ...
    if (raw.length !== 10) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['taxId'], message: 'NIP musi mieć 10 cyfr' })
      return
    }
    ...
    const check = sum % 11
    if (check !== digits[9]) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['taxId'], message: 'Nieprawidłowy NIP (błędna suma kontrolna)' })
    }
...
export default function EdytujKlientaPage() {
  ...
  defaultValues: { name: '', phone: '', email: '', invoiceCity: '', invoicePostalCode: '', invoiceAddress: '', taxId: '', companyName: '', source: '', preferVatInvoice: false, buyerType: 'person', invoiceEmail: '', eInvoiceConsent: false }
  });
    ...
    (async () => {
      try {
        ...
        reset({
          ...
          preferVatInvoice: j.client.buyerType === 'company' ? true : Boolean(j.client.preferVatInvoice),
  ...
  <h1 className="text-2xl font-semibold">{pl.clients.edit} klienta</h1>
      ...
      {loading ? (
        ...
        <form className="space-y-4" onSubmit={handleSubmit(async (data) => {
          ...
          <AddressFields register={register} errors={errors} prefix="invoice" maskPostal />
          {/* Fakturowanie */}
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                <input id="buyer_person" type="radio" value="person" {...register('buyerType', {
                  onChange: (e) => {
                    // przy wyborze osoby – preferVatInvoice = false
                    if (e.target.value === 'person') {
                      (document.getElementById('preferVatInvoice_hidden') as HTMLInputElement | null)!.value = 'false'
                    }
                  }
                })} />
              ...
              <div className="flex items-center gap-2">
                <input id="buyer_company" type="radio" value="company" {...register('buyerType', {
                  onChange: (e) => {
                    // przy wyborze firmy – preferVatInvoice = true (implikowane)
                    if (e.target.value === 'company') {
                      (document.getElementById('preferVatInvoice_hidden') as HTMLInputElement | null)!.value = 'true'
                    }
                  }
                })} />
                <Label htmlFor="buyer_company">Na firmę (preferuje fakturę VAT)</Label>
              </div>
            </div>
            {/* hidden controlled input żeby payload wysłał spójny preferVatInvoice */}
            <input id="preferVatInvoice_hidden" type="hidden" {...register('preferVatInvoice')} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <Label>E‑mail do faktury</Label>
                <Input type="email" {...register('invoiceEmail')} />
                {errors.invoiceEmail && <p className="text-sm text-red-600">{errors.invoiceEmail.message}</p>}
              </div>
              <div className="flex items-center gap-2 mt-6 sm:mt-0">
                <input id="eInvoiceConsent" type="checkbox" className="h-4 w-4" {...register('eInvoiceConsent')} />
