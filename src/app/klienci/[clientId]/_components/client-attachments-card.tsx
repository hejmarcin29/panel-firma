...
import { DownloadCloud, ImagePlus, Loader2 } from "lucide-react";
...
import { uploadClientAttachmentsAction } from "../actions";
import { INITIAL_UPLOAD_CLIENT_ATTACHMENTS_STATE, type UploadClientAttachmentsState } from "../form-state";
...
type ClientAttachmentsCardProps = {
  clientId: string;
...
export function ClientAttachmentsCard({ clientId, clientFullName, attachments }: ClientAttachmentsCardProps) {
  ...
  const latestUploads = attachments.slice(0, 5);
  ...
  return (
    <Card className="rounded-3xl border border-border/60 bg-background/80">
      <CardHeader className="pb-4">
        ...
        <CardDescription>
          Wgrywaj dokumenty i zdjęcia – zostaną zapisane w prywatnym folderze klienta na R2 i pojawią się w repozytorium
          plików.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <form action={formAction} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="attachments" className="text-xs uppercase tracking-wide text-muted-foreground">
              Dodaj zdjęcia / załączniki
            </Label>
            <Input id="attachments" name="files" type="file" multiple className="rounded-xl border-dashed" />
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" className="rounded-full" asChild>
              <Link href="/pliki">Przejdź do repozytorium</Link>
            </Button>
          </div>
        </form>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <p className="font-medium text-foreground">Ostatnio dodane pliki</p>
          ...
          {latestUploads.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Brak załączników dla tego klienta. Dodaj pierwsze pliki, aby pojawiły się w repozytorium.
            </p>
          ) : (
