...
export default function ZleconeMontazePage() {
  ...
  const [installers, setInstallers] = useState<{ id: string; name: string | null; email: string | null }[]>([])
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true)
  const qs = new URLSearchParams({ scope, ...(installerId ? { installerId } : {}) }).toString()
  const r = await fetch(`/api/montaze/zlecone?${qs}`)
        const j = await r.json().catch(() => ({}))
        const Resp = z.object({
          orders: z.array(z.object({
            id: z.string(), clientId: z.string(), clientName: z.string().nullable().optional(),
            installerId: z.string().nullable(), installerEmail: z.string().nullable(), installerName: z.string().nullable(),
            status: z.string(), preMeasurementSqm: z.number().nullable(), scheduledDate: z.number().nullable(), createdAt: z.number(),
          })).optional(),
          error: z.string().optional(),
        })
        const parsed = Resp.safeParse(j)
        if (!r.ok) throw new Error(parsed.success ? (parsed.data.error || 'Błąd') : 'Błąd')
        const data = (parsed.success && parsed.data.orders ? parsed.data.orders : []).map(o => ({
          ...o,
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const j = await r.json().catch(() => ({}))
        const Resp = z.object({ users: z.array(z.object({ id: z.string(), name: z.string().nullable(), email: z.string().nullable() })).optional() })
        const parsed = Resp.safeParse(j)
        if (parsed.success && parsed.data.users) setInstallers(parsed.data.users)
      } catch {}
    })()
  }, [])
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 space-y-4">
      ...
      {!loading && !error && (
        <Card>
          <CardHeader className="pb-2"><CardTitle>Przypisane montaże</CardTitle></CardHeader>
          <CardContent>
            {orders.length === 0 ? (
                  ...
                  <div key={o.id} className="p-3 text-sm flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs opacity-70 mt-0.5 flex items-center gap-2 flex-wrap">
                        <span className="opacity-80">Status:</span> <StatusBadge status={o.status} label={o.status} />
