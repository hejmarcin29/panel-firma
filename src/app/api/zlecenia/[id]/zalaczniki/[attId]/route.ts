...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string; attId: string }> }) {
  try {
    ...
    const [att] = await db.select().from(orderAttachments).where(and(eq(orderAttachments.id, attId), eq(orderAttachments.orderId, id))).limit(1)
    if (!att) return NextResponse.json({ error: 'Nie znaleziono załącznika' }, { status: 404 })
    ...
    try {
      ...
      console.warn('[DELETE attachment] Failed to delete from R2; continuing to remove DB row', e)
    }
