...
export async function PATCH(
  req: Request,
  ctx: { params: Promise<{ id: string; slotId: string; itemId: string }> },
) {
  try {
    ...
    await db
      ...
      .set(update)
      .where(
        and(eq(deliveryItems.id, itemId), eq(deliveryItems.slotId, slotId)),
      );
    revalidatePath(`/zlecenia/${id}`);
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error(
      "[PATCH /api/zlecenia/:id/dostawy/:slotId/pozycje/:itemId] Error",
      err,
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error(
      "[DELETE /api/zlecenia/:id/dostawy/:slotId/pozycje/:itemId] Error",
      err,
    );
    return NextResponse.json({ error: "Błąd serwera" }, { status: 500 });
  }
