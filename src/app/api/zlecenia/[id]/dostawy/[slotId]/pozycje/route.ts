...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string; slotId: string }> }) {
  try {
    ...
    const items = await db.select().from(deliveryItems).where(eq(deliveryItems.slotId, slotId))
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string; slotId: string }> }) {
  try {
    ...
    console.error('[POST /api/zlecenia/:id/dostawy/:slotId/pozycje] Error', err)
    return NextResponse.json({ error: 'Błąd serwera' }, { status: 500 })
  }
}
