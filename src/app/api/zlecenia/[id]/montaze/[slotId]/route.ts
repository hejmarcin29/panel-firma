...
export async function PATCH(
  ...
  try {
    ...
    if ("plannedAt" in parsed.data)
      update.plannedAt = toDate(parsed.data.plannedAt);
    if ("windowStart" in parsed.data)
      update.windowStart = toDate(parsed.data.windowStart);
    if ("windowEnd" in parsed.data)
      update.windowEnd = toDate(parsed.data.windowEnd);
    if ("status" in parsed.data) update.status = parsed.data.status;
    if ("durationMinutes" in parsed.data)
      update.durationMinutes = parsed.data.durationMinutes ?? null;
    ...
    await db
      .update(installationSlots)
      .set(update)
      .where(
        and(
          eq(installationSlots.id, slotId),
          eq(installationSlots.orderId, id),
        ),
      );
