...
export async function GET(req: Request) {
  try {
    ...
    const { clientId, type, status, installerId, limit, withFlags } =
      parsed.data;
    const clauses: SQL[] = [];
    ...
    if (installerId)
      clauses.push(eq(orders.installerId, installerId) as unknown as SQL);
