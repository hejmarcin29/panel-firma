...
import { db } from "@/db";
import { clients, clientNotes, deliverySlots, installationSlots, orders, type Client } from "@/db/schema";
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const [nextInstSlot] = await db
    ...
    .where(and(eq(orders.clientId, id), isNull(orders.archivedAt), isNull(orders.outcome), sql`(${installationSlots.status} in ('planned','confirmed'))`, isNotNull(installationSlots.plannedAt), sql`${installationSlots.plannedAt} >= ${todayStart}`))
    .orderBy(installationSlots.plannedAt)
    .limit(1);
  const [nextInstOrder] = await db
    .select({ t: orders.scheduledDate })
    .from(orders)
    .where(and(eq(orders.clientId, id), isNull(orders.archivedAt), isNull(orders.outcome), eq(orders.type, 'installation'), isNotNull(orders.scheduledDate), sql`${orders.scheduledDate} >= ${todayStart}`))
  ...
  const [nextDelSlot] = await db
    ...
    .where(and(eq(orders.clientId, id), isNull(orders.archivedAt), isNull(orders.outcome), sql`(${deliverySlots.status} in ('planned','confirmed'))`, isNotNull(deliverySlots.plannedAt), sql`${deliverySlots.plannedAt} >= ${todayStart}`))
    .orderBy(deliverySlots.plannedAt)
    .limit(1);
  const [nextDelOrder] = await db
    .select({ t: orders.scheduledDate })
    .from(orders)
    .where(and(eq(orders.clientId, id), isNull(orders.archivedAt), isNull(orders.outcome), eq(orders.type, 'delivery'), isNotNull(orders.scheduledDate), sql`${orders.scheduledDate} >= ${todayStart}`))
    .orderBy(orders.scheduledDate)
    .limit(1);
  ...
  return NextResponse.json({
    client: {
      ...client,
      archivedAt: toMs((client as unknown as { archivedAt?: number | Date | null })?.archivedAt ?? null),
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  await db.transaction(async (tx) => {
    ...
    const [c] = await tx.select({ id: clients.id, archivedAt: clients.archivedAt }).from(clients).where(eq(clients.id, id)).limit(1);
    ...
    if (orderIdsToArchive.length > 0) {
      await tx.update(orders).set({ archivedAt: now }).where(inArray(orders.id, orderIdsToArchive));
      await emitDomainEvent({
