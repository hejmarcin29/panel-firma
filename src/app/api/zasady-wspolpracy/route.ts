...
export async function GET(req: Request) {
  try {
    ...
    if (active) {
      ...
      if (!userId)
        return NextResponse.json({ rule, ack: { acknowledged: false } });
      const ackRows = await db
        .select()
        .from(cooperationRuleAcks)
        .where(
          and(
            eq(cooperationRuleAcks.userId, userId),
            eq(cooperationRuleAcks.version, rule.version),
          ),
        )
        .limit(1);
...
const upsertSchema = z.object({
  ...
  audience: z
    .array(z.enum(["admin", "installer", "architect", "manager"]))
    .default(["installer"]),
