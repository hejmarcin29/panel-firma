  ...
  try {
    ...
    if (!parsed.success)
      return NextResponse.json(
        { error: "Błędne parametry", issues: parsed.error.issues },
        { status: 400 },
      );
    ...
    // Build query
    const where: SQL[] = [
      eq(installerPrivateTasks.userId, ownerId as string) as unknown as SQL,
    ];
    if (status === "open")
      where.push(eq(installerPrivateTasks.done, false) as unknown as SQL);
    if (status === "done")
      where.push(eq(installerPrivateTasks.done, true) as unknown as SQL);
    if (orderId)
      where.push(
        eq(installerPrivateTasks.relatedOrderId, orderId) as unknown as SQL,
      );
