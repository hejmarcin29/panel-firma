  ...
  try {
    ...
    if (parsed.data.pinnedOrderId) {
      const [own] = await db
        .select({ id: orders.id })
        .from(orders)
        .where(eq(orders.id, parsed.data.pinnedOrderId))
        .limit(1);
      if (!own)
        return NextResponse.json(
      ...
      const [assigned] = await db
        ...
        .where(eq(orders.id, parsed.data.pinnedOrderId))
        .limit(1);
      if (!assigned || assigned.installerId !== userId)
        return NextResponse.json(
    ...
    if (existing) {
      await db
        .update(installerPrivatePrefs)
        .set({
          pinnedOrderId: parsed.data.pinnedOrderId ?? null,
          updatedAt: new Date(),
        })
        .where(eq(installerPrivatePrefs.userId, userId));
    } else {
      await db
        .insert(installerPrivatePrefs)
        .values({ userId, pinnedOrderId: parsed.data.pinnedOrderId ?? null });
    }
