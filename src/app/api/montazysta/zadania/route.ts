...
export async function PATCH(req: Request) {
  try {
    ...
    if (rest.dueAt !== undefined) update.dueAt = rest.dueAt ? new Date(rest.dueAt) : null
    update.updatedAt = new Date()
    await db.update(installerPrivateTasks).set(update).where(and(eq(installerPrivateTasks.id, id), eq(installerPrivateTasks.userId, userId)))
