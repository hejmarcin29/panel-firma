...
import {
  installerPrivateNotes,
  installerPrivatePrefs,
  installerPrivateTasks,
} from "@/db/schema";
  ...
  try {
      ...
      .where(
        orderId
          ? and(
              ...
              eq(installerPrivateTasks.relatedOrderId, orderId),
            )
          : eq(installerPrivateTasks.userId, userId),
      )
    ...
    const notes = await db
      ...
      .where(
        orderId
          ? and(
              eq(installerPrivateNotes.userId, userId),
              eq(installerPrivateNotes.relatedOrderId, orderId),
            )
          : eq(installerPrivateNotes.userId, userId),
      )
    ...
    const [prefs] = await db
      .select()
      .from(installerPrivatePrefs)
      .where(eq(installerPrivatePrefs.userId, userId))
      .limit(1);
      ...
      createdAt:
        t.createdAt instanceof Date
          ...
          : typeof t.createdAt === "number"
            ? t.createdAt
            : new Date(t.createdAt as unknown as string).getTime(),
      updatedAt:
        t.updatedAt instanceof Date
          ? t.updatedAt.getTime()
          : typeof t.updatedAt === "number"
            ? t.updatedAt
            : new Date(t.updatedAt as unknown as string).getTime(),
      dueAt:
        t.dueAt == null
          ? null
          : t.dueAt instanceof Date
            ...
            : typeof t.dueAt === "number"
              ? t.dueAt
              : new Date(t.dueAt as unknown as string).getTime(),
    }));
