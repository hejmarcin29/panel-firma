...
export async function GET(req: Request) {
  try {
    ...
    if (!['admin', 'manager', 'architect'].includes(role)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  ...
  const recursive = url.searchParams.get('recursive') === '1'
    ...
    if (!isAllowedPrefix(prefix)) return NextResponse.json({ error: 'Forbidden prefix' }, { status: 403 })
    ...
    const folders = recursive ? [] : (out.CommonPrefixes || []).map((cp) => cp.Prefix || '')
