...
export function MontageDetailsLayout({
  ...
  const mobileTabs = useMemo(() => [
    ...
    { id: 'payments', label: 'Płatności', icon: <FileText className="w-4 h-4" />, content: tabs.payments ? <div className="p-4 pb-24">{tabs.payments}</div> : null },
          ...
          <div className="space-y-6">
              <Tabs value={currentTab === 'overview' ? defaultTab : currentTab} onValueChange={handleTabChange} className="w-full">
                  <TabsList className="flex w-full overflow-x-auto md:flex-wrap">
                      ...
                      {tabs.payments && <TabsTrigger value="payments" className="flex-1">Płatności</TabsTrigger>}
                      {tabs.settlement && <TabsTrigger value="settlement" className="flex-1">Rozliczenie</TabsTrigger>}
