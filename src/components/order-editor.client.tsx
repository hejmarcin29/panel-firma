...
export function OrderEditor({ orderId, defaults }: { orderId: string; defaults: { note?: string|null; preMeasurementSqm?: number|null; installerId?: string|null; scheduledDate?: number|null } }) {
  ...
  const imageItems = useMemo(() => attachments
    .filter(a => (a.mime || '').startsWith('image/'))
    .map(a => ({ src: `/api/pliki/r2/proxy?key=${encodeURIComponent(a.key)}` })), [attachments])
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { /* noop */ } finally { setAttLoading(false) }
    })()
  }, [orderId])
    ...
    <div className="grid gap-3">
      ...
      <div className="mt-6 border-t border-black/10 pt-4 dark:border-white/10">
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          <select value={category} onChange={(e) => setCategory(e.target.value as Attachment['category'])} className="h-8 rounded-md border border-black/15 bg-transparent px-2 text-sm dark:border-white/15">
        ...
        <div
          ...
          onDrop={(e) => { e.preventDefault(); setDragOver(false); const files = Array.from(e.dataTransfer.files || []); if (files.length) void handleFiles(files) }}
        ...
        >
          Przeciągnij i upuść pliki tutaj lub <label className="underline cursor-pointer"><input type="file" multiple className="hidden" onChange={(e) => {
            const files = Array.from(e.currentTarget.files || [])
            if (files.length) void handleFiles(files)
            e.currentTarget.value = ''
          }} /> wybierz z dysku</label>
          {uploading && <div className="mt-2 text-xs opacity-70">Wysyłanie…</div>}
        </div>
          ...
          {attLoading ? (
            ...
            <ul className="grid gap-2">
              {attachments.map((a, idx) => (
                <li key={a.id} className="flex items-center justify-between gap-3 rounded-md border border-black/10 p-2 text-sm dark:border-white/10">
                    ...
                    <div className="h-10 w-10 overflow-hidden rounded bg-black/5 dark:bg-white/10">
                      {a.mime?.startsWith('image/') ? (
                        <img
                          ...
                          onError={(e) => { (e.currentTarget as HTMLImageElement).src = `/api/pliki/r2/proxy?key=${encodeURIComponent(a.key)}` }}
                        />
                      ) : (
                        <FilePreview url={`/api/pliki/r2/proxy?key=${encodeURIComponent(a.key)}`} contentType={a.mime || undefined} />
                      )}
                    </div>
                    <div className="min-w-0">
                      <div className="truncate font-medium">{a.key.split('/').slice(-1)[0]}</div>
                      <div className="text-xs opacity-70">{a.category} · {a.mime || 'plik'} · {formatSize(a.size)}</div>
                    </div>
                  </div>
                  <div className="shrink-0 space-x-2">
                    {a.mime?.startsWith('image/') && (
                      <button type="button" className="rounded-md border border-black/15 px-2 py-1 text-xs hover:bg-black/5 dark:border-white/15 dark:hover:bg-white/10" onClick={() => setLightbox({ open: true, index: idx })}>Podgląd</button>
                    )}
                    <a href={a.publicUrl} target="_blank" className="rounded-md border border-black/15 px-2 py-1 text-xs hover:bg-black/5 dark:border-white/15 dark:hover:bg-white/10">Pobierz</a>
                    <button type="button" className="rounded-md border border-red-500/50 px-2 py-1 text-xs text-red-600 hover:bg-red-500/10 dark:text-red-400" onClick={() => deleteAttachment(a.id)}>Usuń</button>
                  </div>
                </li>
              ))}
            </ul>
        ...
        {lightbox.open && imageItems.length > 0 && (
          <ImageLightbox images={imageItems} open={lightbox.open} onClose={() => setLightbox({ open: false, index: 0 })} />
        )}
      </div>
    </div>
  )
  ...
  async function handleFiles(files: File[]) {
    ...
    try {
      for (const file of files) {
        // presign
        const pres = await fetch(`/api/zlecenia/${orderId}/zalaczniki/presign`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ filename: file.name, mime: file.type || 'application/octet-stream', size: file.size, category })
        })
        const pj = await pres.json().catch(() => ({}))
        if (!pres.ok) throw new Error(pj?.error || 'Błąd presign')
        // upload to R2
        const up = await fetch(pj.url, { method: 'PUT', headers: { 'Content-Type': file.type || 'application/octet-stream' }, body: file })
        if (!up.ok) throw new Error('Błąd wysyłania do R2')
        // finalize metadata
        const fin = await fetch(`/api/zlecenia/${orderId}/zalaczniki`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ key: pj.key, publicUrl: pj.publicUrl, category, mime: file.type || null, size: file.size })
        })
      ...
      const msg = e instanceof Error ? e.message : 'Nie udało się wysłać plików'
      toast({ title: 'Błąd', description: msg, variant: 'destructive' })
    } finally {
      setUploading(false)
    }
  ...
  async function deleteAttachment(attId: string) {
    ...
    try {
      const r = await fetch(`/api/zlecenia/${orderId}/zalaczniki/${attId}`, { method: 'DELETE' })
      const j = await r.json().catch(() => ({}))
