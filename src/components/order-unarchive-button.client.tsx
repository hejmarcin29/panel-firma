...
export function OrderUnarchiveButton({ id }: { id: string }) {
  ...
  async function unarchive() {
    try {
      setLoading(true)
      const r = await fetch(`/api/zlecenia/${id}/archiwum`, { method: 'DELETE' })
  ...
  return (
    <>
      <button onClick={() => setConfirmOpen(true)} disabled={loading} className="inline-flex h-8 items-center rounded-md border border-black/15 px-2 text-xs hover:bg-black/5 disabled:opacity-60 dark:border-white/15 dark:hover:bg-white/10">Cofnij archiwum</button>
      <AlertDialog
        open={confirmOpen}
