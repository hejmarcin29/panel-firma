  ...
  async function handleUpload() {
    ...
    try {
      const res = await fetch("/api/pliki/r2/presign", {
        ...
        body: JSON.stringify({ contentType: file.type || "application/octet-stream", size: file.size, filename: file.name, prefix: "uploads/" }),
      })
  ...
  return (
    <div className="space-y-2 p-3 border rounded">
      ...
      <div className="flex gap-2">
        <button onClick={handleUpload} disabled={!file || busy} className="px-3 py-1 bg-black text-white rounded disabled:opacity-50">
      ...
      {url && (
        <div className="text-sm">
          Gotowe: <a className="text-blue-600 underline" href={url} target="_blank" rel="noreferrer">{url}</a>
        </div>
      )}
