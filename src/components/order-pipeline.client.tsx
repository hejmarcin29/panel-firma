...
export function OrderPipeline({ orderId, type, stage }: Props) {
  ...
  const currentLabel = useMemo(() => opts.find(o => o.value === value)?.label ?? '— wybierz —', [opts, value])
        ...
        {opts.map((o) => (
          ...
          >
            <div className="flex items-center gap-2">
              <div className={`h-2 w-2 rounded-full ${value === o.value ? 'bg-black dark:bg-white' : 'bg-black/30 dark:bg-white/30'}`} />
              <span>{o.label}</span>
            </div>
          </DropdownItem>
        ))}
