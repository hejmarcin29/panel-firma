...
export function PdfViewer({ url, scale = 1.2 }: { url: string; scale?: number }) {
  ...
  pdf.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${ver}/pdf.worker.min.js`;
    ...
    (async () => {
      try {
  const doc = await (pdf as PdfJsModule).getDocument(url).promise as unknown as { getPage: (n: number) => Promise<{ getViewport: (opts: { scale: number }) => { height: number; width: number } }> };
  ...
  const pageWithRender = page as unknown as { render: (params: { canvasContext: CanvasRenderingContext2D; viewport: { height: number; width: number } }) => { promise: Promise<void> } };
  await pageWithRender.render({ canvasContext: ctx, viewport }).promise;
      } catch (e: unknown) {
        if (!cancelled) setError(e instanceof Error ? e.message : 'Nie udało się wczytać PDF');
      }
    })();
