    ...
    <div className="flex gap-1 flex-wrap">
      {onConfirm && (
        <button
          ...
          onClick={onConfirm}
          disabled={disabled}
          className="inline-flex h-7 items-center rounded-md border border-black/15 px-2 text-xs hover:bg-black/5 dark:border-white/15 dark:hover:bg-white/10"
        >
          Potwierdź
        </button>
      )}
...
export function DeliverySlotsList({
  ...
  const patch = async (slotId: string, payload: DeliveryPatch) => {
    setBusy(slotId);
    try {
      const res = await fetch(`/api/zlecenia/${orderId}/dostawy/${slotId}`, {
        method: "PATCH",
    ...
    <div>
      ...
      <div className="space-y-2">
        {slots.map((s) => (
          <div
            key={s.id}
            className="rounded-md border border-black/10 dark:border-white/10 p-2 text-sm flex items-center justify-between flex-wrap gap-2"
          >
            <div className="space-y-0.5">
              <div>
                <span className="opacity-70">Plan:</span> {fmt(s.plannedAt)}{" "}
                {s.status ? (
                  <span className="ml-1 rounded bg-black/10 px-1.5 py-0.5 text-[10px] uppercase tracking-wide dark:bg-white/10">
                    {s.status}
                  </span>
                ) : null}
              </div>
              <div className="opacity-70">
                Okno: {fmt(s.windowStart)} – {fmt(s.windowEnd)}
              </div>
              {(s.carrier || s.trackingNo) && (
                <div className="opacity-70">
                  {s.carrier || ""} {s.trackingNo ? `(${s.trackingNo})` : ""}
                </div>
              )}
            ...
            <ActionButtons
              onConfirm={
                s.status === "planned"
                  ? () => patch(s.id, { status: "confirmed" })
                  : undefined
              }
              onComplete={
                s.status !== "completed" && s.status !== "canceled"
                  ? () => patch(s.id, { status: "completed" })
                  : undefined
              }
              onCancel={
                s.status !== "canceled" && s.status !== "completed"
                  ? () => patch(s.id, { status: "canceled" })
                  : undefined
              }
              onReschedule={
                s.status !== "completed" && s.status !== "canceled"
...
export function InstallationSlotsList({
  orderId,
  slots,
}: {
  orderId: string;
  ...
  const patch = async (slotId: string, payload: InstallationPatch) => {
    setBusy(slotId);
    try {
      const res = await fetch(`/api/zlecenia/${orderId}/montaze/${slotId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
        ...
        {slots.map((s) => (
          ...
          >
            <div className="space-y-0.5">
              ...
              <div className="opacity-70">
                Montażysta: {s.installerName || s.installerId || "—"}
              </div>
              {s.durationMinutes != null && (
                <div className="opacity-70">Czas: {s.durationMinutes} min</div>
              )}
              {s.note && <div className="opacity-70">{s.note}</div>}
            </div>
            <ActionButtons
