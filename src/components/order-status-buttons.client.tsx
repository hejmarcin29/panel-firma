...
export function OrderStatusButtons({ id, status }: Props) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        toast({
          ...
          description:
            (data && (data.error || data.message)) || `Kod: ${r.status}`,
          variant: "destructive",
        });
        return;
      }
  ...
  if (next.length === 0)
    return <div className="text-xs opacity-70">Brak dostępnych przejść.</div>;
    ...
    <div className="flex flex-wrap gap-2">
      {next.map((s) => (
        ...
        >
          {loading === s
            ? "Zapisywanie…"
            : `Ustaw: ${(pl.orders.statuses as Record<string, string>)[s] || s}`}
        </button>
      ))}
    </div>
  );
