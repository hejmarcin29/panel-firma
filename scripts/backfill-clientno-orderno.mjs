...
const tx = db.transaction(() => {
  ...
  const clients = db
    .prepare("SELECT id FROM clients ORDER BY created_at ASC")
    .all();
  let no = 10;
  const updClient = db.prepare("UPDATE clients SET client_no=? WHERE id=?");
  for (const c of clients) {
    const cur = db
      .prepare("SELECT client_no FROM clients WHERE id=?")
      .get(c.id);
  ...
  const ordersByClient = db.prepare(
    "SELECT id FROM orders WHERE client_id=? ORDER BY created_at ASC",
  );
  const updOrder = db.prepare("UPDATE orders SET seq=?, order_no=? WHERE id=?");
  const clientIds = db
    .prepare("SELECT DISTINCT client_id as id FROM orders")
    .all();
  for (const row of clientIds) {
    ...
    for (const o of list) {
      const cur = db
        .prepare("SELECT seq, order_no FROM orders WHERE id=?")
        .get(o.id);
      if (cur && cur.seq != null && cur.order_no) {
        seq = Math.max(seq, cur.seq + 1);
        continue;
      }
