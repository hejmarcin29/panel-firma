...
try {
  info.checks.nextauthUrlLooksValid = /^https?:\/\//i.test(info.env.NEXTAUTH_URL || '');
  info.checks.nextauthSecretStrong = (info.env.NEXTAUTH_SECRET_len || 0) >= 32;

  // List dirs
  try { info.ls.app = fs.readdirSync(process.cwd()); } catch (e) { info.errors.push(['lsApp', String(e)]); }
  try { fs.mkdirSync(info.paths.dataDir, { recursive: true }); info.ls.data = fs.readdirSync(info.paths.dataDir); } catch (e) { info.errors.push(['prepareDataDir', String(e)]); }
