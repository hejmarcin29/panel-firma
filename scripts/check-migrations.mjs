...
const appliedNumbers = new Set(Array.from(appliedTags).map(t => t.split('_')[0]));
const lastAppliedNumber = Math.max(...Array.from(appliedNumbers).map(n => parseInt(n, 10)));
...
for (let i=1;i<sorted.length;i++) {
  if (sorted[i] !== sorted[i-1] + 1) {
    warn(`Gap detected between ${sorted[i-1].toString().padStart(4,'0')} and ${sorted[i].toString().padStart(4,'0')}. Ensure missing numbers are not uncommitted.`);
  }
}
...
if (unapplied.length) {
  ...
  if (blocking.length) {
    ...
    warn(`Found file(s) with numbers <= last applied but missing in journal (ignored by Drizzle): ${unapplied.map(u=>u.file).join(', ')}`);
  }
}
...
try {
  ...
  try { execSync('git rev-parse --verify origin/main', {stdio:'ignore'});} catch { baseRef = 'HEAD~1'; }
  const diffStat = execSync(`git --no-pager diff --name-only ${baseRef} -- ${schemaPath}`).toString().trim();
...
if (schemaChanged) {
  ...
  try {
    let baseRef = 'origin/main';
    try { execSync('git rev-parse --verify origin/main', {stdio:'ignore'});} catch { baseRef = 'HEAD~1'; }
    migrationsDiff = execSync(`git --no-pager diff --name-only ${baseRef} -- drizzle/`).toString();
  } catch {}
  if (!/\.sql/.test(migrationsDiff)) {
    fail('schema.ts changed but no new migration (.sql) detected. Run: npx drizzle-kit generate; npx drizzle-kit migrate');
  }
}
